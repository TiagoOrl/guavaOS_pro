...
static int heap_validade_table(void* ptr, void* end, struct heap_table* table)
{
    ...
    size_t total_blocks = table_size / GUAVAOS_HEAP_BLOCK_SIZE;
...
{
    return ((unsigned int)ptr % GUAVAOS_HEAP_BLOCK_SIZE) == 0;
}
...
int heap_create(struct heap* heap, void* ptr, void* end, struct heap_table* table)
{
    ...
    heap->saddr = ptr;  // set the heap to the start address provided
    heap->table = table;
...
{
    if (val % GUAVAOS_HEAP_BLOCK_SIZE == 0)
        return val;

    val = (val - (val % GUAVAOS_HEAP_BLOCK_SIZE));
    val += GUAVAOS_HEAP_BLOCK_SIZE;
...
void* heap_malloc(struct heap* heap, size_t size)
{
    ...
    uint32_t total_blocks = aligned_size / GUAVAOS_HEAP_BLOCK_SIZE;
