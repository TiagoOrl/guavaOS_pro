...
{
    for (int i = 0; i < GUAVAOS_MAX_FILESYSTEMS; i++)
    {
        if (filesystems[i] == 0)
        {
...
void fs_insert_filesystem(struct filesystem* filesystem)
{
    ...
    if (!fs)
    {
        print("Problem inserting filesystem", 3); while(1) {}
    }
...
static int file_new_descriptor(struct file_descriptor** desc_out)
{
    ...
    for (int i = 0; i < GUAVAOS_MAX_FILE_DESCRIPTORS; i++)
    {
...
{
    if (fd <= 0 || fd >= GUAVAOS_MAX_FILE_DESCRIPTORS)
        return 0;
